 ...
 // ------------------------------------------------------------------
  ...
  module.md2Tex =  function(string, levelStyles, renderImpl) {
    ...
    if (renderImpl.delImpl)
      marked.setOptions({delRenderer: renderImpl.delImpl});
    if (renderImpl.verbatimImpl)
      marked.setOptions({verbatimRenderer: renderImpl.verbatimImpl});
    if (renderImpl.linkImpl)
      marked.setOptions({linkRenderer: renderImpl.linkImpl});
    if (renderImpl.imageImpl)
      marked.setOptions({imageRenderer: renderImpl.imageImpl});
