      ...
      text = text.replace(/([ \n]*)\{\{Attachements:(.*?)\}\}([ \n]*)/g, function myFunction(_, s1, x, s2){
          ...
          try {
              if (x.charAt(0) === '*') {
                ...
                adjacent = settings.noadjacent == true ? false : true;
                x = x.substr(1)
              }
              x = parseInt(x)
              if (x > 0) {
                  ...
                  if (attachements && attachements.Content[x] && attachements.Content[x].Files) {
                      ...
                      let _index = preformats.push(`\\placeoriginal{${source}}{${localfile}}{${marked.parseInline(attachements.Content[x].Captions[0] || "")}}{${s}}{${_landscape}}{${marked.parseInline(attachements.Content[x].Captions[1] || "")}}{${docId}-${x+1}}{${adjacent}}{${settings.nocaptiondefault}}`)
