...
module.exports = function(log, slack) {
 ...
 // ------------------------------------------------------------------
      ...
      for (let index = 0; index < data[key].length; index++) {
        if (index === options.selection ||  options.selection === false) {
          ...
          if (download(`${source.src}?backend=true`, localfile)) {
            data[key][index].localfile = options.relative
  ...
  module.resolveEmbeddedImages_v2 = (data, key, language, field, replacement, options) => {
    ...
    let text = language === false ? data[key] ?? undefined :  data[key]?.[language] ?? undefined
    if (text !== undefined) {
      // Attachements
      const regex = new RegExp(`\\{\\{${field}:(.*?)\\}\\}`, 'g')
      text = text.replace(regex, function myFunction(_, x){
        x = parseInt(x) - 1 // {{Attachement}} Syntax starts with 1
        if (x >= 0 && data[field] &&  data[field][x]) {
          ...
          data[field][x].isEmbedded = true
          return replacement
                    .replace('{{image-path}}', data[field][x].localfile)
                    .replace('{{caption-text}}', language === false ? data[field]?.[x]?.captions : data[field]?.[x]?.captions[language])
        }
      })
      if (language === false) {
        data[key] = text
      } else {
        data[key][language] = text
      }
    }
  }
 ...
 */
  ...
  module.md2Typst = function(data, key, language, replacement) {

    let result = language === false ? (data[key] ?? undefined) :  data[key]?.[language] ?? undefined
  ...
  // Store protected content (code, links, etc.) with placeholders
  const protected = {
    codeBlocks: [],
    inlineCode: [],
    links: [],
  ...
  // 1. Protect HTML comments
  result = result.replace(/<!--[\s\S]*?-->/g, (match) => {
    const id = protected.html.length;
    protected.html.push('');  // Remove comments
    return `__HTML_${id}__`;
  });

  // 2. Protect code blocks (fenced and indented)
  result = result.replace(/```(\w+)?\n([\s\S]*?)```/g, (match, lang, code) => {
    const id = protected.codeBlocks.length;
    protected.codeBlocks.push(`\`\`\`${lang || ''}\n${code.trimEnd()}\n\`\`\``);
  ...
  result = result.replace(/\n((?:(?:    |\t).*\n)+)/g, (match, code) => {
    ...
    protected.codeBlocks.push(`\`\`\`\n${cleaned.trim()}\n\`\`\``);
    return `\n__CODEBLOCK_${id}__\n`;
  });

  // 3. Protect inline code
  result = result.replace(/`([^`\n]+)`/g, (match, code) => {
    const id = protected.inlineCode.length;
    protected.inlineCode.push(`\`${code}\``);
    return `__INLINECODE_${id}__`;
  });

  // 4. Protect images first (before links, since they start with !)
  result = result.replace(/!\[([^\]]*)\]\(([^)]+?)(?:\s+"([^"]*)")?\)/g, (match, alt, url, title) => {
    const id = protected.images.length;

    const suffix = url.split('.').pop();
    const localfile = path.join(module.downloadPath, `${getMD5Hash(url)}.${suffix}`);
    if (download(url, localfile)) {
      const finalPath = path.relative(module.engineSettings.workspace, localfile)
      protected.images.push(replacement
            .replace('{{image-path}}', finalPath)
            .replace('{{caption-text}}', alt));
    return `__IMAGE_${id}__`;
  ...
  // 5. Protect links
  result = result.replace(/\[([^\]]+)\]\(([^)]+?)(?:\s+"([^"]*)")?\)/g, (match, text, url, title) => {
    const id = protected.links.length;
    // Check if text contains formatting
    const processedText = processInlineFormatting(text);
    protected.links.push(`#link("${url}")[${processedText}]`);
  ...
  result = result.replace(/<(https?:\/\/[^>]+)>/g, (match, url) => {
    const id = protected.links.length;
    protected.links.push(`#link("${url}")`);
    return `__LINK_${id}__`;
  });

  // 7. Process headings (with proper escaping)
  result = result.replace(/^(#{1,6})\s+(.+)$/gm, (match, hashes, text) => {
    const level = '='.repeat(hashes.length);
    const processedText = processInlineFormatting(text);
    return `#lock[${level} ${processedText}]`;
  });

  // Alternative heading syntax (underlines)
  result = result.replace(/^(.+)\n=+\s*$/gm, (match, text) => {
  ...
  // 8. Horizontal rules
  result = result.replace(/^(?:[-*_]){3,}\s*$/gm, '#line(length: 100%)');

  // 9. Blockquotes
  result = result.replace(/^>\s+(.+)$/gm, (match, text) => {
    const processedText = processInlineFormatting(text);
    return `#quote[${processedText}]`;
  });
  ...
  result = result.replace(/^((?:>\s+.+\n?)+)/gm, (match) => {
    ...
    const processedText = processInlineFormatting(content);
    return `#quote[\n${processedText}\n]`;
  });

  // 10. Lists - Unordered
  result = result.replace(/^([ \t]*)([-*+])\s+(.+)$/gm, (match, indent, marker, text) => {
    const level = indent.length / 2;
    const spacing = '  '.repeat(level);
    const processedText = processInlineFormatting(text);
    return `${spacing}- ${processedText}`;
  });

  // 11. Lists - Ordered
  result = result.replace(/^([ \t]*)(\d+)\.\s+(.+)$/gm, (match, indent, num, text) => {
    const level = indent.length / 2;
    const spacing = '  '.repeat(level);
    const processedText = processInlineFormatting(text);
    return `${spacing}+ ${processedText}`;
  ...
  // 12. Tables (basic support)
  result = result.replace(/^\|(.+)\|\n\|[-:\s|]+\|\n((?:\|.+\|\n?)+)/gm, (match, header, rows) => {
    const headerCells = header.split('|').map(c => c.trim()).filter(c => c);
    const rowLines = rows.trim().split('\n');
    const dataRows = rowLines.map(row => 
      row.split('|').map(c => c.trim()).filter(c => c)
    );
    
    let table = '#table(\n  columns: ' + headerCells.length + ',\n';
    table += '  [*' + headerCells.map(h => escape(h)).join('*], [*') + '*],\n';
    dataRows.forEach(row => {
      table += '  [' + row.map(c => escape(c)).join('], [') + '],\n';
    });
    table += ')';
    return table;
  });

  // 13. Process inline formatting (done in a specific order)
  result = processInlineFormatting(result);

  // 14. Escape remaining special characters in plain text
  // Split by lines and escape text that isn't already Typst syntax
  const lines = result.split('\n');
  result = lines.map(line => {
    // Skip lines that are already Typst commands or contain placeholders
    if (
      line.match(/^[=\-+]/) ||
      line.includes('__') ||
      line.match(/^#(line|quote|table|image|link|strike|lock)/) ||
      line.trim() === ''
    ) {
      return line;
    }
    // For remaining lines, escape only if they're pure text
    //if (!line.match(/[*_~]/) && !line.includes('__')) {
      return escape(line);
    //}
    //return line;
  }).join('\n');

  // 15. Restore protected content
  protected.html.forEach((content, i) => {
    result = result.replace(`__HTML_${i}__`, content);
  });

  protected.images.forEach((content, i) => {
    result = result.replace(`__IMAGE_${i}__`, content);
  });

  protected.links.forEach((content, i) => {
    result = result.replace(`__LINK_${i}__`, content);
  });

  protected.inlineCode.forEach((content, i) => {
    result = result.replace(`__INLINECODE_${i}__`, content);
  });

  protected.codeBlocks.forEach((content, i) => {
    result = result.replace(`__CODEBLOCK_${i}__`, content);
  });
  if (language === false) {
    data[key] = result.trim()
  } else {
    data[key][language] = result.trim()
  }
}

/**
 * Process inline formatting (bold, italic, strikethrough, etc.)
...
function processInlineFormatting(text) {
  ...
  // Skip if text contains placeholders (already protected)
  if (result.includes('__')) {
    return result;
  }

  // Strikethrough (must be before other formatting)
  result = result.replace(/~~(.+?)~~/g, (match, content) => {
    const processed = processInlineFormatting(content);
    return `#strike[${processed}]`;
  });
  ...
  result = result.replace(/(?<!_)_(.+?)_(?!_)/g, (match, content) => {
    const escaped = escape(content);
    return `_${escaped}_`;
  });
