module.exports = function(log, slack) {
 ...
 // ------------------------------------------------------------------
  ...
  module.resolveAttachements = async function(contributionId, attachementId, forceAltImage) {
    var d = await module.getRf(`contribution/${contributionId}`, {status:'published', flat: 'true'})
    let x = attachementId - 1
    if (d.Attachements && d.Attachements[x] && d.Attachements[x].Original) {
      let source = d.Attachements[x]
