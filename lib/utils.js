...
module.exports = function(log, slack) {
  ...
  module.nj         = nj;
  module.log        = log;
  module.downloadPath = `${__dirname}/../.downloads` //temp.mkdirSync('download_files');

  if (!fs.existsSync(module.downloadPath)) {
    fs.mkdirSync(module.downloadPath, { recursive: true });
  }
  ...
  var download = async function(url, dest) {
    if (fs.existsSync(dest) === false) {
      ...
      log.info(`  - Skipped Download: ${url} / ${dest}`)
    }
  }
