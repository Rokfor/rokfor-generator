module.exports = function(log, slack) {
  ...
  module.linkImpl = function (href, title, text) {
    //log.info(href, title, text);
    if (href !== null)  href  = href.replace(/\\/g, "\\%5C");
    if (title !== null) title = module.texEscape(title);//.replace(/\\/g, "");
    if (text !== null)  text  = module.texEscape(text);//.replace(/\\/g, "");
