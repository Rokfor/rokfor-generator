 ...
 // ------------------------------------------------------------------
  ...
  var preformatting = function(text) {
      ...
      text = text.replace(/:imagereference\[(.*?)?\]/g, function myFunction(_, x){
        let _index = preformats.push(`\\ref{${module.texEscape(x.trim())}}`);
        return `---preformatter---${(_index-1)}-`;              
      });
      ...
      text = text.replace(/(\s*)\{\{Attachements:(.*?)\}\}(\s*)/g, function myFunction(_, s1, x, s2){
          try {
              if (x > 0) {
                  x--;
                  if (attachements && attachements.Content[x] && attachements.Content[x].Files) {
                      ...
                      let _index = preformats.push(`\\placeoriginal{${attachements.Content[x].Files.Original}}{${localfile}}{${module.texEscape(attachements.Content[x].Captions[0])}}{${s}}{${_landscape}}{${module.texEscape(attachements.Content[x].Captions[1])}}{${docId}-${docName}-${x+1}}`)
