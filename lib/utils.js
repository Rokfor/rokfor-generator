 ...
 // ------------------------------------------------------------------
  ...
  var linkImpl = function (href, title, text) {
    if ((typeof href === "string" && href) && (!text || text.replace(/\\/g, "") == decodeHtmlEntities(href.replace(/\\/g, "")))) {
      ...
      if (!text) {
        href  = module.texmodule.escapeMD(href.replace(/\\/g, ""));      
        return '\\url{' + href + '}';
      }
      // return text as \url{text} if href is the same as text
      else {
        text  = module.texmodule.escapeMD(text.replace(/\\/g, ""));      
        return '\\url{' + text + '}';
      }
    }
    // continue here for \hrefs with {href} and {text}
    if (typeof href === "string" && href)  href  = module.texmodule.escapeMD(href.replace(/\\/g, ""));
    if (typeof title === "string" && title) title = module.texmodule.escapeMD(title);//.replace(/\\/g, "");
    if (typeof text === "string" && text)  text  = module.texmodule.escapeMD(text.replace(/\\/g, ""));//.replace(/\\/g, "");
  ...
  var verbatimImpl = function (code, lang, escaped) {
    ...
    return [
      '\\begin{verbatim}',
      TexRenderer.htmlUnmodule.escapeMD(code),
      '\\end{verbatim}'
    ].join(NEWLINE) + NEWLINE;
  };
 ...
 */
  ...
  var preformatting = function(text) {
      ...
      text = text.replace(/:mark\[(.*?)?\]\{reference="(.*?)"\}/g, function myFunction(_, __, x){
        let _index = preformats.push(`\\label{${module.texmodule.escapeMD(x.trim())}}`);
        return `---preformatter---${(_index-1)}-`;              
      });
      // Page Reference (pointing to Mark): [reference:]
      text = text.replace(/:reference\[(.*?)?\]\{reference="(.*?)"\}/g, function myFunction(_, __, x){
        let _index = preformats.push(`\\pageref{${module.texmodule.escapeMD(x.trim())}}`);
        return `---preformatter---${(_index-1)}-`;              
      });
      ...
      text = text.replace(/:imagereference\[(.*?)?\]\{reference="(.*?)"\}/g, function myFunction(_, __, x){
        ...
        let _index = preformats.push(`\\ref{${module.texmodule.escapeMD(`${_parts[0]}-${_parts.pop()}`)}}`);
        return `---preformatter---${(_index-1)}-`;              
      });
      // Extra Link : [extra:]
      ...
      text = text.replace(/\\?\[(in|index):(.*?)\\?\]/g, function myFunction(_, func, x){
        let _index = preformats.push(`\\index{${module.texmodule.escapeMD(x.trim())}}`)
        return `---preformatter---${(_index-1)}-`;              
      });
      // Mark: [mark:]
  ...
  module.htmlUnescape = function(html) {
    return TexRenderer.htmlUnmodule.escapeMD(html);
  }
  ...
  module.texEscape = function(text) {
    ...
    return postformatting(TexRenderer.texmodule.escapeMD(preformatting(text)));
  }
