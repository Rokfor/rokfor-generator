module.exports = function(log, slack) {
  ...
  temp.track();
  const downloadPath = temp.mkdirSync('download_files');
 ...
 // ------------------------------------------------------------------
  ...
  module.md2Tex =  function(string, levelStyles, renderImpl, attachements, settings, docName, docId) {
    docName = docName || "";
    docId = docId || "";
  ...
  var preformatting = async function(text) {
      ...
      text = text.replace(/\\\n/g, ' ----force-new-line--- ');
      text = text.replace(/\\\\\\\\/g, ' ----force-new-line--- '); 
      ...
      text = text.replace(/:fn\[([\S\s]*?)?\]/g, function myFunction(_, x){
        let _index = preformats.push(`\\footnote{${marked.parseInline(x.trim())}}`)
        return `---preformatter---${(_index-1)}-`;              
      });
      // Index: [in:] or [index:]
      ...
      text = text.replace(/:imagereference\[(.*?)?\]/g, function myFunction(_, x){
      ...
      // Attachements
      let _localDownloads = {};
      if (attachements && attachements.Content && attachements.Content.length > 0) {
        for (let index = 0; index < attachements.Content; index++) {
          const x = attachements.Content[index];
          let suffix = x.Files.Original.split('.').pop();
          let escapedFile = x.Files.Original.replace(/[\W_]+/g,"");
          let localfile = path.join(downloadPath, `${escapedFile}.${suffix}`);
          await download(x.Files.Original, localfile)
          _localDownloads[x.Files.Original] = localfile;
        }
      }
  
      text = text.replace(/( *)\{\{Attachements:(.*?)\}\}( *)/g, function myFunction(_, s1, x, s2){
          try {
              if (x > 0) {
                  x--;
                  if (attachements && attachements.Content[x] && attachements.Content[x].Files) {
                      let localfile = _localDownloads[attachements.Content[x].Files.Original];
                      ...
                      let _index = preformats.push(`\\placeoriginal{${attachements.Content[x].Files.Original}}{${localfile}}{${module.texEscape(attachements.Content[x].Captions[0])}}{${s}}{${_landscape}}{${module.texEscape(attachements.Content[x].Captions[1])}}{${docId}-${x+1}}`)
                      return `---preformatter---${(_index-1)}-${s1 && s2 ? ' ' : ''}`;   
                  }    
              }
          }
          catch(err) {
              let _index = preformats.push(err)
              return `---preformatter---${(_index-1)}-`;                    
          }
      })
