  ...
  module.md2Tex =  function(string, levelStyles) {
    ...
    var preformatting = function(text) {
      ...
      text = text.replace(/\\\[description:([\S\s]*?)\\\]/gm, function myFunction(_, x){
        let _string = '\\begin{description}';
        let t_string = x.replace(/\{(.*?)\}/g, function myFunction(_, x){
          return '\\item[{' + x.trim().replace(/\\n/, '') + '}]';
        });
        _string += marked(t_string);
        _string += '\\end{description}';
        let _index = preformats.push(_string);
