module.exports = function(log, slack) {
 ...
 // ------------------------------------------------------------------
  ...
  var imageImpl = function (href, title, text) {
    let NEWLINE = '\r\n';
    let localfile = href.replace(/[\W_]+/g,"");
    return [
      NEWLINE,
  ...
  var preformatting = function(text) {
          ...
          try {
              if (x > 0) {
                  ...
                  if (attachements && attachements.Content[x] && attachements.Content[x].Files) {
                      let localfile = attachements.Content[x].Files.Original.replace(/[\W_]+/g,"");
                      ...
                      let  _landscape  = attachements.Content[x].Sizes[0] > attachements.Content[x].Sizes[1] ? 'landscape' : 'portrait';
      ...
      text = text.replace(/\\?\[pdf:(.*?)\\?\]/g, function myFunction(_, x){
        ...
        let localfile = href.replace(/[\W_]+/g,"") + ".pdf";
