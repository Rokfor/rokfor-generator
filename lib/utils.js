          ...
          try {
              ...
              if (x > 0) {
                  ...
                  if (attachements && attachements.Content[x] && attachements.Content[x].Files) {
                      ...
                      let _floating = attachements.Content[x].Captions[4] || 'false'

                      let _index = preformats.push(`\\placeoriginal{${source}}{${localfile}}{${marked.parseInline(attachements.Content[x].Captions[0] || "")}}{${s}}{${_landscape}}{${marked.parseInline(attachements.Content[x].Captions[1] || "")}}{${docId}-${x+1}}{${adjacent}}{${settings.nocaptiondefault}}{${_floating}}`)
              ...
              log.info(`  - Attachements Error: ${err}`)
              let _index = preformats.push('')
              return `---preformatter---${(_index-1)}-`;                    
          }
      })
