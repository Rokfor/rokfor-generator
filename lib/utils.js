module.exports = function(log, slack) {
 ...
 // ------------------------------------------------------------------
  ...
  module.md2Tex =  function(string, options = {}) {
    ...
    let spaceAfterFootnote = options.disableAfterFootnote === true ? "" : " " // Default: Add Space after a footnote.
          ...
          try {
              if (x > 0) {
                  ...
                  if (attachements && attachements.Content[x] && attachements.Content[x].Files) {
                      ...
                      let _index = preformats.push(`\\placeoriginal{${source}}{${localfile}}{${marked.parseInline(attachements.Content[x].Captions[0])}}{${s}}{${_landscape}}{${marked.parseInline(attachements.Content[x].Captions[1])}}{${docId}-${x+1}}`)
