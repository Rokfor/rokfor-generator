module.exports = function(log, slack) {
 ...
 // ------------------------------------------------------------------
  ...
  module.md2Tex =  function(string, levelStyles, renderImpl, attachements) {
          ...
          try {
              if (x > 0) {
                  ...
                  if (attachements && attachements.Content[x] && attachements.Content[x].Files) {
                      ...
                      let d = new Date();
                      let localfile = d.getTime() + "_" + r;
                      let _index = preformats.push(`\\placeoriginal{${attachements.Content[x].Files.Original}}{${localfile}}{${module.texEscape(attachements.Content[x].Captions[0])}}`)
                      return `---preformatter---${(_index-1)}-`;   
                  }    
              }
          }
