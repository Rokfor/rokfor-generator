module.exports = function(log, slack) {
  ...
  module.md2Tex =  function(string, levelStyles) {
    ...
    var preformatting = function(text) {
      ...
      // No Indent: [hangindent:text]

      text = text.replace(/\\\[hangindent:([\S\s]*?)\\\]/gm, function myFunction(_, x){
        // Convert everything from Markdown to TEX
        let t_string = marked(x);
        let _index = preformats.push(`\\begin{flushleft}\\noindent\\setlength{\\hangindent}{\\parindent} ${t_string}\\end{flushleft}`);
        return `---preformatter---${(_index-1)}-`;              
      });
