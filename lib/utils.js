          ...
          try {
              if (x > 0) {
                  ...
                  if (attachements && attachements.Content[x] && attachements.Content[x].Files) {
                      ...
                      let d = new Date();
                      let localfile = d.getTime() + "_" + r;
                      let settings = settings.size1 ? 1 :
                                        settings.size2 ? 2 :
                                          settings.size3 ? 3 : 0;

                      let _index = preformats.push(`\\placeoriginal{${attachements.Content[x].Files.Original}}{${localfile}}{${module.texEscape(attachements.Content[x].Captions[0])}}{${settings}}`)
                      return `---preformatter---${(_index-1)}-`;   
                  }    
              }
          }
