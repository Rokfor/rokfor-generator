...
module.exports = function(config, log, slack) {
  ...
  module.clone = function(name, repo, mode) {
    ...
    if (mode === "github") {
      if (config.github.password) myURL.password = config.github.password;
      if (config.github.user) myURL.username = config.github.user; 
    }
    if (mode === "gitlab") {
      if (config.gitlab.password) myURL.password = config.gitlab.password;
      if (config.gitlab.user) myURL.username = config.gitlab.user; 
    }
