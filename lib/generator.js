...
module.exports = function(config, log, slack) {
  ...
  module.creator = function() {
    ...
    try {
      delete require.cache[require.resolve(`${templatedir}${utils.gdata.name}/lib/globals.js`)];
    } catch (err) {
      log.info(`  - Could not clean globals.js`)
    }
    try {
      delete require.cache[require.resolve(`${templatedir}${utils.gdata.name}/lib/processIssueOptions.js`)];
    } catch (err) {
