...
module.exports = function(config, log, slack) {
  ...
  module.runLocal = function(project, params) {
    log.info(`  - Incoming Project ${project}`)
    utils.gdata   = {
      name:      project,
      id:        1,
      callback:  params.CallbackUrl,
      config:    { Book: [], Issue: [], Chapter: [], Template: [] }, 
      selection: { Mode: 'issues', Value: params.Issue },
      files:     {},
      fileinfo:  {}
    };
  ...
  module.creator = function() {
    ...
    try {
      const _globals = `${templatedir}${utils.gdata.name}/lib/globals.js`
      if (fs.existsSync(_globals)) {
        delete require.cache[require.resolve(_globals)];
        log.info(`  - clean ${_globals}`)
      } else {
        log.info(`  - could not find ${_globals}`)
      }
    } catch (err) {
      log.info(`  - Could not clean globals.js`)
    }
    try {
      const _processIssueOptions = `${templatedir}${utils.gdata.name}/lib/processIssueOptions.js`
      if (fs.existsSync(_processIssueOptions)) {
        delete require.cache[require.resolve(_processIssueOptions)];
        log.info(`  - clean ${_processIssueOptions}`)
      } else {
        log.info(`  - could not find ${_processIssueOptions}`)
      }
    } catch (err) {
      log.info(`  - Could not clean processIssueOptions.js`)
    }
  ...
  module.subcreator = async function(plugin, output, index) {
    ...
    for (var i = 0, len = outputconfig.length; i < len; i++) {
      ...
      if (page.template !== false) {
        try {
          ...
          console.log(`  - Nunjuck Render Error:  ${e}`)
          log.error(e);
    ...
    temp.mkdir('pdfcreator', function(err, dirPath) {
      ...
      if (keepTexFile == true) {
        let _texFile = (module.local_only === true) 
          ? 'out.tex'
          : path.join(temp.mkdirSync('tex_files'), 'out.tex');
        log.info(`  - [TEX] Stored in ${_texFile}`);
      ...
      oStream.on('finish', function () {
        if (module.local_only === true) {
          console.log(`save output to: ${__dirname}/../output.pdf`)
          fs.copyFileSync(outputPath, `${__dirname}/../output.pdf`);
          deferred.reject('PDFINFO Local Exit');
        }
        try {
