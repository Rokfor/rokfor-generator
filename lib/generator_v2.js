  ...
  module.subcreator = async function(configuration, pagename) {
    ...
    if (engine === 'latex') {
      ...
      try {
        ...
        // Run the compile in a throwaway child process that exits afterwards.
        // typst-ts-node-compiler leaks native memory (fonts, decoded images,
        // the comemo cache) that V8's GC cannot reclaim, so compiling in this
        // long-lived server process makes RSS grow by gigabytes per run. A
        // short-lived child process hands that memory back to the OS on exit.
        const contentFile = path.join(dirPath, 'input.typ')
        const jobFile     = path.join(dirPath, 'typst-job.json')
        await fsp.writeFile(contentFile, sourcFileComplete)
        ...
        await new Promise((resolve, reject) => {
          execFile(
            process.execPath,
            [path.join(__dirname, 'typst-compile-worker.js'), jobFile],
            { maxBuffer: 64 * 1024 * 1024 },
            (err, stdout, stderr) => {
              if (stdout) log.info(`[${engine}] ${stdout.toString().trim()}`)
              if (err) {
