  ...
  module.subcreator = async function(configuration, pagename) {
    ...
    if (engine === 'latex') {
      ...
      try {
        ...
        // Typst keeps a process-wide (comemo) memoization cache that is NOT
        // tied to this compiler instance. Without evicting it, memoized ASTs,
        // fonts, images and layout frames accumulate across every run and are
        // never freed. Evict on each compile so used memory is released.
        // maxAge 10 is the documented value for non-watch tools.
        try { $typst.evictCache(10); } catch (e) { /* best-effort */ }
      }
