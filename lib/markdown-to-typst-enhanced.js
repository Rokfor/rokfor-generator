...
class MarkdownToTypstConverter {
  ...
  convertInline(text) {
    ...
    text = text.replace(/\\\*/g, '\x00ESC_ASTERISK\x00');
    text = text.replace(/\\_/g, '\x00ESC_UNDERSCORE\x00');
    ...
    text = text.replace(/\x00ESC_ASTERISK\x00/g, '\\*');
    text = text.replace(/\x00ESC_UNDERSCORE\x00/g, '\\_');
